...
// 使用 any 类型作为返回值，因为 hastscript 的返回类型比较复杂
/**
...
export function GithubCardComponent(
  ...
  children: any[]
	...
	const repo: string = properties.repo;
	const cardUuid: string = `GC${Math.random().toString(36).slice(-6)}`; // Collisions are not important
	...
	const nScript = h(
		...
		`
      fetch('https://api.github.com/repos/${repo}', { referrerPolicy: "no-referrer", cache: "force-cache", next: { revalidate: 3600 }, headers: {'Authorization': 'token ${token}', 'Accept': 'application/vnd.github.v3+json'} }).then(response => response.json()).then(data => {
